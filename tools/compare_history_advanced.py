#!/usr/bin/env python3
"""
Enhanced script to compare CSV files with additional features:
- Detailed statistics
...
class CSVComparator:
    """A class to handle CSV file comparisons with various options."""
    ...
    def export_differences(self, results: Dict, output_dir: str = "comparison_output"):
    ...
    def analyze_statistics(self, columns: List[str]) -> Dict:
        """Generate detailed statistics about the files."""
        ...
        for col in columns:
            if col in df1.columns:
                stats['file1'][f'{col}_unique'] = df1[col].nunique()
                stats['file1'][f'{col}_nulls'] = df1[col].isnull().sum()
            
            if col in df2.columns:
                stats['file2'][f'{col}_unique'] = df2[col].nunique()
                stats['file2'][f'{col}_nulls'] = df2[col].isnull().sum()
...
def main():
    ...
    parser.add_argument('file1', nargs='?', default='../data/history_export.csv', 
                       help='Path to first CSV file')
    parser.add_argument('file2', nargs='?', default='../data/history.csv',
    ...
    parser.add_argument('--ignore-dates', action='store_true',
                       help='Ignore date columns in comparison')
    ...
    try:
        ...
        if args.ignore_dates:
            compare_columns = [col for col in compare_columns 
        ...
        if results:
            ...
            # Print statistics
            stats = comparator.analyze_statistics(compare_columns)
            print("\n" + "="*80)
            print("FILE STATISTICS")
            print("="*80)
            print(f"{comparator.file1_name}: {stats['file1']['total_rows']} rows, "
                  f"{stats['file1']['memory_usage']} bytes")
            print(f"{comparator.file2_name}: {stats['file2']['total_rows']} rows, "
                  f"{stats['file2']['memory_usage']} bytes")
